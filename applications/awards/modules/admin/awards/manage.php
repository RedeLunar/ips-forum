...
class _manage extends \IPS\Node\Controller
{
    ...
    protected function addAward()
    {
        ...
        {
            $form->add( new \IPS\Helpers\Form\Editor( 'awarded_reason', isset($award->id) ? $award->reason : NULL, FALSE, array( 'app' => 'awards', 'key' => 'Awards', 'autoSaveKey' => 'awards-new-reason', 'attachIds' => ( !isset($award->id) ? NULL : array( $award->id ) ) ), NULL, NULL, NULL, 'reason_show' ) );
        }
        ...
        if ( $values = $form->values() )
        {
            ...
            {
                $message = \IPS\Member::loggedIn()->language()->get( 'awards_others_err' );
            }

            if ( $award->can( 'add', $member ) AND !$award->can( 'self', $member ) AND $giver->member_id == $member->member_id )
            {
                $message = \IPS\Member::loggedIn()->language()->get( 'awards_self_err' );
            }

            if( $award->obtainable_enabled AND $award->obtainable )
            {
                $message = \IPS\Member::loggedIn()->language()->get( 'awards_obtainable_err' );
            }

            if ($message) {
                \IPS\Output::i()->error( $message, '1CL1/1' );
            }
    ...
    protected function edit()
    {
        ...
        $form->add( new \IPS\Helpers\Form\Editor( 'awarded_reason', $awarded->reason ? $awarded->reason : NULL, FALSE, array( 'app' => 'awards', 'key' => 'Awards', 'autoSaveKey' => 'awards-new-reason', 'attachIds' => ( !isset($awarded->id) ? NULL : array( $awarded->id ) ) ), NULL, NULL, NULL, 'reason_show' ) );
    ...
    protected function remove()
    {
        ...
        $form->add( new \IPS\Helpers\Form\TextArea( 'award_removed_reason', NULL, \IPS\Settings::i()->award_settings_require_reason, array() ) );
