...
class _databases extends \IPS\Node\Controller
{
	...
	public function add()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['cms_database_import'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=cms&module=databases&controller=databases&do=import' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ) ) ) );
			}
			else
			{
	...
	public function import()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			\IPS\Http\Url::internal( 'app=cms&module=databases&controller=databases&do=import' )->setQueryString( array( 'file' => \IPS\Request::i()->file, 'key' =>  \IPS\Request::i()->key ) ),
			function( $data )
			{
				/* Open XML file */
