...
class _Invoice extends \IPS\Patterns\ActiveRecord
{
	...
	public function summary()
	{
		...
		foreach ( $taxableAmounts as $taxId => $amount )
		{
			...
			$return['tax'][ $taxId ] = array( 'name' => '', 'rate' => $rate, 'amount' => $amount ); // "name" is no longer used. It is definef here only to not break custom themes. Get the title from the ID instead.
	...
	{		
		...
		$referredBy = isset( $this->guest_data['referred_by'] ) ? \IPS\Member::load( $this->guest_data['referred_by'] ) : NULL;
		...
		/* Were we referred? */
		if ( $referredBy and $referredBy->member_id )
		{
			\IPS\Db::i()->insert( 'nexus_referrals', array(
				'member_id'		=> $memberToSave->member_id,
				'referred_by'	=> $referredBy->member_id,
				'amount'			=> '',
			) );
		}
		...
		if ( $this->status === static::STATUS_PAID )
		{
			...
			foreach ( $this->items as $k => $item )
			{
				...
				if ( $item->payTo and !$item->payTo->inGroup( explode( ',', \IPS\Settings::i()->nexus_no_commission ) ) )
				{
					...
					$credits = $item->payTo->cm_credits;
					$credits[ $this->currency ]->amount = $credits[ $this->currency ]->amount->subtract( $recipientAmounts['recipient_final']->amount );
					$item->payTo->cm_credits = $credits;
					$item->payTo->save();
					...
					\IPS\Email::buildFromTemplate( 'nexus', 'commissionRevoked', array( $this, $item, $recipientAmounts['recipient_final'] ), \IPS\Email::TYPE_TRANSACTIONAL )
						->send(
							$item->payTo,
							array_map(
								function( $contact )
	...
	public function unpaidConsequences()
	{
		...
		if ( \count( $purchases ) )
		{
			...
			{
				$return['purchases'][] = \IPS\Theme::i()->getTemplate( 'purchases', 'nexus' )->link( $purchase );
			}
		}
