...
class _product extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		else
		{
			...
			{
				\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate('store')->package( $package, $item, $this->_getForm( $package, $inCart, TRUE ), array_sum( $inCart ), $shippingMethods, $itemDataForShipping, $locationType, $renewalTerm );	
			}
			...
			if( $price !== NULL )
			{
				\IPS\Output::i()->jsonLd['package']	= array(
					...
					'offers'		=> array(
										'@type'			=> 'Offer',
										'availability'	=> ( $package->stock > 0 OR $package->stock == -1 ) ? 'http://schema.org/InStock' : 'http://schema.org/OutOfStock',
		...
		if ( $values = $form->values() )
		{
			...
			if ( \IPS\Request::i()->isAjax() && isset( \IPS\Request::i()->stockCheck ) )
			{
				...
				if ( !empty( $renewOptions ) )
				{
					...
					$return['renewal'] = \IPS\Member::loggedIn()->language()->addToStack( 'and_renewal', FALSE, array( 'sprintf' => array(
				...
				{
					$return['price'] = \IPS\Theme::i()->getTemplate( 'store', 'nexus' )->price( $data['price'], FALSE, FALSE );
				}
