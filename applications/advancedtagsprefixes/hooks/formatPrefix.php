...
/* End Hook Data */
	...
	function prefix( $encoded, $text )
	{
		try
		{
			...
			$return .= '" title="';
			$sprintf = array($text);
			$return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'find_tagged_content', ENT_DISALLOWED, 'UTF-8', FALSE ), FALSE, array( 'sprintf' => $sprintf ) );
			...
			{
				$return .= htmlentities( $text, ENT_QUOTES | ENT_DISALLOWED, 'UTF-8', FALSE );
			}
			$return .= $prefix->post;
