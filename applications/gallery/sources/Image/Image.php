...
\IPS\Content\MetaData
{
	...
	{
		return $this->_data['small_file_name'] ?: $this->_data['masked_file_name'];
	}
	...
	public function fetchNextOrPreviousImages( $count, $direction = 'DESC' )
	{
		...
		if( !$this->imagesSameSort )
		{
			$where['date']	= array( static::$databasePrefix . $sortBy . " {$dir}= ?", $this->$sortBy );
			return iterator_to_array( \IPS\gallery\Image::getItemsWithPermission( $where, static::$databasePrefix . $sortBy . ' ' . $direction . ', image_id ' . $direction, $count ) );
		}
		else
		{
			unset( $where['id'] );
			...
			foreach( \IPS\gallery\Image::getItemsWithPermission( $where, static::$databasePrefix . $sortBy . ' ' . $direction . ', image_id ' . $direction, NULL ) as $image )
			{
				if( !$seen )
				{
					if( $image->id == $this->id )
	...
	public function processForm( $values )
	{
		...
		if ( isset( $values['image_description'] ) )
		{
			...
			$sendFilterNotifications = $this->checkProfanityFilters( FALSE, $this->_new );
	...
	public function buildThumbnails( $file=NULL )
	{
		...
		/* Create the various thumbnails */
		$largeImage				= \IPS\File::create( 'gallery_Images', 'large.' . $file->originalFilename, $this->createImageFile( $file, explode( 'x', \IPS\Settings::i()->gallery_large_dims ), FALSE, \in_array( 'large', $watermarks ) ), $file->container );
