...
class _convert extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		if( !\IPS\CONVERTERS_DEV_UI )
		{
			...
			{
				$url = \IPS\Http\Url::internal( "app=convert&module=manage&controller=convert&do=runStep&id=" . $app->app_id );
	...
	protected function _getNextStep( $app )
	{
		...
		if( \count( $applicationsToCheck ) )
		{
			foreach( $applicationsToCheck as $appToCheck )
			{
				...
				/* Set up cache array */
				if( !isset( $_SESSION['convertCountRows'][ $masterAppId ][ $appToCheck->sw ] ) )
				{
					$_SESSION['convertCountRows'][ $masterAppId ][ $appToCheck->sw ] = array();
				}
				...
				if( !$appToCheck->parent AND $softwareClass::canConvertSettings() AND isset( $appToCheck->_session['more_info']['convertSettings'] ) AND $appToCheck->_session['more_info']['convertSettings']['convert_settings'] AND !\count( $appToCheck->_session['completed'] ) )
				{
					...
					$_SESSION['convertCountRows'][ $masterAppId ][ $appToCheck->sw ][ $method ] = 1;
				}
				...
				foreach( $menuRows as $row )
				{
					...
					if ( !isset( $_SESSION['convertCountRows'][ $masterAppId ][ $appToCheck->sw ][ $row['step_method'] ] ) )
					{
						/* Only count the rows if the cache is missing */
						$counts = $softwareClass->getLibrary()->getDatabaseRowCounts( array( $row ), FALSE, TRUE );
						$_SESSION['convertCountRows'][ $masterAppId ][ $appToCheck->sw ][ $row['step_method'] ] = $counts[0]['source_rows'];
					}
					...
					/* Check whether the dependency was set to be converted */
					foreach( $row['dependencies'] as $k => $v )
					{
						if( empty( $appToCheck->_session['more_info'][ $v ][ $menuRows[ $v ]['step_title'] ] ) )
						{
	...
	protected function runStep()
	{
		...
		$redirectUrl = \IPS\Http\Url::internal( "app=convert&module=manage&controller=convert&do=runStep&id={$app->app_id}" );
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect( 
			...
			function( $data ) use ( $app )
			{
				...
				if( $convertData['method'] == 'convertSettings' )
				{
					...
					return array( 0, sprintf( \IPS\Member::loggedIn()->language()->get( 'converted_x_of_x' ), $percentage, 1, 1, \IPS\Member::loggedIn()->language()->addToStack( '_convert_settings' ) ), $percentage );
				}
			...
			function() use ( $app )
			{
				...
				{
					\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=convert&module=manage&controller=convert&do=finish&wasConfirmed=1&id=" . ( $app->parent ?: $app->app_id ) ) );
				}
			}
		 );
	}
